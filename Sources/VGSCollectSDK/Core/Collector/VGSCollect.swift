...
/// Update SKD with custom idenitifier.
public struct VGSCollectOptions {

	public var customIdentifier: String = ""

	public var loggerConfiguration: VGSCollectLoggingConfiguration = VGSCollectLoggingConfiguration()
}
...
public class VGSCollect {
    ...
    public var textFields: [VGSTextField] {
      return storage.textFields
		...
		internal var collectOptions: VGSCollectOptions?

		internal var logger: VGSCollectLogger
	...
	public init(id: String, environment: String, hostname: String? = nil, satellitePort: Int? = nil, collectOptions: VGSCollectOptions? = nil) {
			self.collectOptions = collectOptions
			var logger = VGSCollectLogger.shared
			if let options = collectOptions {
				logger = VGSCollectLogger()
				logger.loggerPrefix = options.customIdentifier
			}
      ...
      self.formAnalyticsDetails = VGSFormAnanlyticsDetails.init(formId: formId, tenantId: tenantId, environment: regionalEnvironment)
		...
		self.apiClient = APIClient(tenantId: id, regionalEnvironment: environment, hostname: hostname, formAnalyticsDetails: formAnalyticsDetails, satellitePort: satellitePort, logger: logger)
    ...
    ///   - hostname: `String` object, custom Hostname, if not set, data will be sent to Vault Url. Default is `nil`.
	  ///   - satellitePort: `Int?` object, custom port for satellite configuration. Default is `nil`. **IMPORTANT! Use only with .sandbox environment! Hostname should be specified for valid http://localhost or in local IP format http://192.168.X.X**.
	public convenience init(id: String, environment: Environment = .sandbox, dataRegion: String? = nil, hostname: String? = nil, satellitePort: Int? = nil, collectOptions: VGSCollectOptions? = nil) {
      let env = Self.generateRegionalEnvironmentString(environment, region: dataRegion)
      self.init(id: id, environment: env, hostname: hostname, satellitePort: satellitePort, collectOptions: collectOptions)
    }
